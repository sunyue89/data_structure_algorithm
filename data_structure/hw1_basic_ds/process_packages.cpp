...
public:
    ...
    //pass in request and return response
    Response Process(const Request &request) {
	Response resp(false,0);
	if(finish_time_.size()==0){
		...
		if(request.arrival_time >= finish_time_.back()){
			...
			resp.dropped = false;
			resp.start_time = finish_time_.back();
		...
		if(request.arrival_time >= finish_time_.front()){
			finish_time_.pop();
			resp.dropped = false;
			resp.start_time = std::max (finish_time_.back(),request.arrival_time);
			finish_time_.push(resp.start_time + request.process_time);
		}else{
			resp.dropped = true;
		}
